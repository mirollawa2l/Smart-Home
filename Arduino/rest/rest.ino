...
// SoftwareWire for the second LCD (Smoke & Fire)
SoftwareWire myWire(A2, A3);
LiquidCrystal_I2C lcd2(0x3F, 16, 2); // Address remains the same but uses SoftwareWire
...
void loop() {
  ...
  if (currentMillis - lastSmokeCheckTime >= smokeCheckInterval) {
    ...
    myWire.beginTransmission(0x3F); // Use SoftwareWire explicitly
    lcd2.setCursor(0, 0);
    lcd2.print("Smoke:");
  ...
  if (currentMillis - lastFireCheckTime >= fireCheckInterval) {
    ...
    myWire.beginTransmission(0x27); // Use SoftwareWire explicitly
    lcd2.setCursor(0, 0);
    lcd2.print("Flame:");
